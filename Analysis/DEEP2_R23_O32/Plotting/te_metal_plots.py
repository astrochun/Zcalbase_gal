...
def Jiang_comparison():
    ...
    pdf_pages = PdfPages(fitspath + 'comparsion_Jiang_Zcal.pdf')
    ...
    for aa in range(len(jR23_MACT)):
        jR23_MACT[aa] = a + b * der_OH_MACT[aa] + c * (der_OH_MACT[aa] * der_OH_MACT[aa])\
                        - d * (e + der_OH_MACT[aa]) * der_O32_MACT[aa]
