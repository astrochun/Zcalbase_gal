...
def n_times_binned(fitspath, dataset, pdf_pages, outfile, n_split, R23, O32, SNR3, data3, galinbin):
    ...
    for ii in range(n_bins):
        ...
        # We have to take the length of the range of n_split because n_split is of type int
        ...
        for jj in range(n_split):
            ...
            # This index gives the positions of all the R23 values relative to the O32 values over the entire R23 bin

            # Now let's start storing our data into variables to use later
            # These two values give the lowest O32 and R23 value set for each bin
            O32_minimum[ii, jj] = O32_values_perbin[0]
            R23_minimum[ii, jj] = R23_sort0[bin_start[ii]]

            # These two give the median R23 and O32 value for each bin
            ...
            # Gives the index for the spectra in each bin and is used later loop over and get the galaxies
            ...
            xBar[(ii*n_split)+jj] = np.log10(np.average(R23[N_bin_idx]))

            # Gives the average O32 value for each bin
            yBar[(ii*n_split)+jj] = np.log10(np.average(O32_values_perbin))
