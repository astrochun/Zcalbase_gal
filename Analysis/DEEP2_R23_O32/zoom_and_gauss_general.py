...
### Performs the Gaussian Fitting on All Emission Lines
### Called everytie by the general functions
...
# Plotting Zoomed
def zoom_gauss_plot(dataset, fitspath, tab, stack2D, dispersion,
                    s2, wave, working_wave, lineflag,  y_correction='', line_type = '',
                    outpdf='', line_name=''):   #s, a, c, s1, a1, s2, a2,
    ...
    if line_type == 'Single': 
        n=  ('Flux_Gaussian', 'Flux_Observed', 'S/N','Sigma', 'Median', 'Norm', 'RMS',  'Center', 'Pos_Sig', 'Pos_Amp', 'Const')
    ...
    if line_type == 'Balmer' or line_type == 'Oxy2': 
         n=  ('Flux_Gaussian', 'Flux_Observed', 'S/N','Sigma', 'Median', 'Norm', 'RMS', 'Center', 'Pos_Sig', 'Pos_Amp', 'Const', 'Neg_Sig', 'Neg_Amp')
         n = tuple([line_name + '_' + val for val in n])
    ...
    out_ascii_single_original = fitspath+'/'+dataset+'_Average_R23_O32_Values_original.tbl'
    ...
    n2= ('bin_ID', 'N_stack')
    tab1 = Table([ID,  N_gal_array], names=n2)
    asc.write(tab1, out_ascii_single, format='fixed_width_two_line')
...
def zm_general(dataset, fitspath, stack2D, wave, lineflag, dispersion, y_correction,s,a,c,s1,a1,s2,a2,tab):
    ...
    for ii in range(len(lambda0)):
        ...
        if line_type[ii] == 'Single':
            ...
            print(outpdf)
            zoom_gauss_plot(dataset, fitspath, tab, stack2D, dispersion,
                            s2, wave,lambda0[ii], lineflag, y_correction, line_type[ii], outpdf, line_name[ii])  #s, a, c, s1, a1, s2, a2,
        ...
        if line_type[ii] == 'Oxy2': 
            ...
            print(outpdf)
            zoom_gauss_plot(dataset, fitspath, tab, stack2D, dispersion,
                            s2, wave, lambda0[ii], lineflag, y_correction, line_type[ii], outpdf, line_name[ii])
