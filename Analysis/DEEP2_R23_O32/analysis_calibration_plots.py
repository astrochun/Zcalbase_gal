...
y_calibration = 'jiang'   #This is a keyword that will be passed in in order to specify which calibration we want to compare to
...
lR23 = det_R23  #,nandet_R23] #,der_R23,der_R23_MACT][det_R23,nandet_R23,der_R23,der_R23_MACT]
lO32 = det_O32  #,nandet_O32] #,der_O32,der_O32_MACT][det_O32,nandet_O32,der_O32,der_O32_MACT]
OH   = det_OH  #,nandet_OH] #, der_OH, der_OH_MACT][det_OH,nandet_OH, der_OH, der_OH_MACT]

#green_peas_calibration.main(lR23, lO32, OH, out_pdf, n_bins = 6, xra = [0.3,1.5], yra = [6.5,9.10], marker=['D','X','3','4'], label=['Detection','Non-Dectection','DEEP2', 'MACT'], fit=False, silent=False, verbose=True)



#Given R23_composite and O32_composite, calculate the metallicity according to the Jiang calibration, and plot against Zcalbase_gal derived metallicities#
...
for qq in range(len(det_ID)): ax.annotate(det_ID[qq], (x_plus[qq], det_OH[qq]), fontsize= '12')
for aa in range(len(det_ID)): ax.annotate(det_ID[aa], (x_minus[aa],det_OH[aa]), fontsize= '12')
plt.show()
